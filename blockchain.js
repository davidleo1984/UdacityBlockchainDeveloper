    ...
    getStarsByWalletAddress(address) {
        ...
        return new Promise((resolve, reject) => {
            for (let index = 1; index < self.chain.length; index++) {
                var parsedBody = JSON.parse(hex2ascii(self.chain[index].body));
                if (parsedBody.owner == address) {
                    stars.push(parsedBody);
                }
            }
            resolve(stars);
        });
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (let index = 0; index < self.chain.length; index++) {
                if (await self.chain[index].validate() == false) {
                    errorLog.push({ "height": index, "error": "Not vaild." });
                }
                if (self.chain[index].previousBlockHash != previousBH) {
                    errorLog.push({ "height": index, "error": "previousBlockHash not match." });
                }
                previousBH = self.chain[index].hash;
            }
            resolve(errorLog);
        });
    }
